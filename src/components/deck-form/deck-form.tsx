...
export function DeckForm({
  ...
  const [currentStep, setCurrentStep] = useState<"details" | "cards">(
    "details"
  );
  const [announcement, setAnnouncement] = useState("");
  const [createdDeckDetails, setCreatedDeckDetails] = useState<{
    id: string
    title: string
    description: string
    cardCount: number
  ...
  useEffect(() => {
    const updateFormValues = async () => {
      ...
      // Trigger validation after updating values
      await trigger()
    }
    updateFormValues()
  }, [cards, setValue, trigger])
  ...
  const handleNavigate = (direction: "prev" | "next") => {
    if (direction === "next" && currentCardIndex < cards.length - 1) {
      ...
      setAnnouncement(
        `Mostrando cartão ${currentCardIndex + 2} de ${cards.length}`
      );
    } else if (direction === "prev" && currentCardIndex > 0) {
      setCurrentCardIndex((prev) => prev - 1);
      setAnnouncement(`Mostrando cartão ${currentCardIndex} de ${cards.length}`);
    }
  };
  ...
  const handleCreateDeck = async (data: FormValues) => {
    ...
    try {
      ...
      onError?.(error.message || "Error creating deck")
    } finally {
      setIsLoading(false)
    }
  };
    ...
    <TooltipProvider>
      ...
      <div className="container max-w">
        <Card>
          <CardHeader>
            <CardDescription>
              Crie seu deck de cartões para praticar repetição espaçada.
            </CardDescription>
          </CardHeader>
          <CardContent>
            <form id="deck-form" onSubmit={handleSubmit(handleCreateDeck)}>
                  ...
                  <div className="flex items-center justify-between">
                    ...
                    <div className="flex items-center gap-4">
                      ...
                      <Tooltip>
                        ...
                        <TooltipContent>Pré-visualizar este cartão</TooltipContent>
                      </Tooltip>
                      <Tooltip>
                        <TooltipTrigger asChild>
                          <Button
                        ...
                        <TooltipContent>Adicionar um novo cartão</TooltipContent>
                      </Tooltip>
                    </div>
                  </div>
                  ...
                  <AnimatePresence mode="wait">
                    <CardForm
                      ...
                      onCorrectAnswerChange={(value) => {
                        const newCards = [...cards];
                        newCards[currentCardIndex].correctAnswerIndex =
        ...
        {createdDeckDetails && (
          <SuccessDialog
            ...
            deckId={createdDeckDetails.id}
            title={createdDeckDetails.title}
            description={createdDeckDetails.description}
            cardCount={createdDeckDetails.cardCount}
            createdAt={createdDeckDetails.createdAt}
          />
        )}
      </div>
    </TooltipProvider>
